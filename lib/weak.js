...
const { WeakTag, ObjectInfo } = require('ene-node-gyp-build')(path.join(__dirname, '..'));
...
function weak(obj, callback) {
	...
	if (list === undefined) map.set(obj, [tag]);
	else list.push(tag);
	if (typeof callback === 'function') info.on('dead', callback);
...
const proxyHandler = {
	...
	getOwnPropertyDescriptor(info, key) {
		const target = info.target;
		return target === undefined ? undefined : Object.getOwnPropertyDescriptor(target, key);
	},
	defineProperty(info, key, descriptor) {
		const target = info.target;
		return target === undefined ? false : Object.defineProperty(target, key, descriptor);
	},
	has(info, key) {
		const target = info.target;
	...
	ownKeys(info) {
		const target = info.target;
		return target === undefined ? [] : Object.getOwnPropertyNames(target).concat(Object.getOwnPropertySymbols(target));
	},
};
